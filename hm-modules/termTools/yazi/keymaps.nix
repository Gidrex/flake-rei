...
in {
  programs.yazi.keymap.mgr.prepend_keymap = [
    ...
    (mkBind [ "e" "d" ] ''shell '${ripdrag} "$@" -A 2>/dev/null &' --confirm''
      "Drag & Drop file all select files as one")
    (mkBind [ "e" "o" ] "shell 'onlyoffice-desktopeditors \"$@\"'"
    ...
    (mkBind "O" "plugin open-with-cmd block"
      "Open with command in the terminal")
    (mkBind [ "<C-c>" ] "plugin close-and-restore-tab close_to_right"
      "Close current tab")
    (mkBind [ "<C-t>" ] "plugin close-and-restore-tab restore"
      "Restore closed tab")

    # UI controls
    (mkBind [ "T" "t" ] "plugin toggle-pane min-preview" "Hide or show preview")
    (mkBind [ "T" "T" ] "plugin toggle-pane max-preview"
  ...
  ] ++ relative-motions ++ tab-switching;
}
